...
void loop() {
  ...
  if(start_ctr == 0){
    ...
    if((hr_24_ctr == 1 && dt.hour == feed_hour[1].toInt() && format_minutes(String(dt.minute)).toInt() == feed_minute[1].toInt() && format_seconds(String(dt.second)).toInt() == feed_second[1].toInt()) || feed_ctr == 1){
      feed_ctr = 1;
      feedfish();
    }
  }
